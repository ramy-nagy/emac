<x-app-layout>
    ...
    <form action="{{route("project.model", 'Fitting_Rec_Reducer') }}" method="get" enctype="multipart/form-data">
        @csrf
    ...
    <div class="col-md-12">
        <div class="px-4  py-3 mb-3 text-gray-600 dark:text-gray-300 rounded-lg shadow-md dark:bg-gray-800">
            Project NO :: {{$project_id ?? ''}} - Fitting_Rec_Reducer.
            <form action="{{route('Rec_Reducer')}}" method="post">
                @csrf
                ...
                <div class="row mt-3">
                    ...
                    <div class=" col-md-6 border border-dark p-2 my-2 mr-1" style="width:60%">
                        <div class="why dark:bg-gray-800" style="width: 240px">Fitting data</div>
                        <label class=" text-sm col-md-2">
                            <span class="text-gray-700 dark:text-gray-400">Width1 (mm)</span>
                            <input type="number" step="0.01" min="0" id="Rec_Elbow_width1" value="0"
                                name="width1"
                                class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input">
                        </label>
                        <label class=" text-sm col-md-2">
                            <span class="text-gray-700 dark:text-gray-400">Depth1 (mm)</span>
                            <input type="number" step="0.01" min="0" id="Rec_Elbow_depth1" name="depth1"
                                value="0"
                                class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input">
                        </label>
                        <label class=" text-sm col-md-2">
                            <span class="text-gray-700 dark:text-gray-400">Width2 (mm)</span>
                            <input type="number" step="0.01" min="0" id="Rec_Elbow_width2" name="width2"
                                value="0"
                                class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input">
                        </label>
                        <label class=" text-sm col-md-2">
                            <span class="text-gray-700 dark:text-gray-400">Depth2 (mm)</span>
                            <input type="number" step="0.01" min="0" id="Rec_Elbow_depth2" name="depth2"
                                value="0"
                                class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input">
                        </label>
                ...
                <table class="table whitespace-no-wrap datatable" id="example_1">
                    <thead>
                        <tr
                            ...
                            <th class="py-3">Width1</th>
                            <th class="py-3">Depth1</th>
                            <th class="py-3">Width2</th>
                            <th class="py-3">Depth2</th>
                    ...
                    <tbody class=" divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @forelse ($project->FittingRecReducers as $RecDuct)
                        <tr class="text-gra y-700 dark:text-gray-400">
                            <td class="py-3 border border-secondary border-end">{{$RecDuct->tag_no ?? ''}}</td>
                            <td class="py-3 border border-secondary border-end">{{$RecDuct->width1 ?? ''}}</td>
                            <td class="py-3 border border-secondary border-end">{{$RecDuct->depth1 ?? ''}}</td>
                            <td class="py-3 border border-secondary border-end">{{$RecDuct->width2 ?? ''}}</td>
                            <td class="py-3 border border-secondary border-end">{{$RecDuct->depth2 ?? ''}}</td>
                            ...
                            <td>
                                <a href="{{ route('delete',['model'=>'FittingRecReducer', 'id'=> $RecDuct->id]) }}"
    ...
    <script>
        ...
        $("input[type='number']").change(function(){
            var Rec_Elbow_width1 = parseFloat($("#Rec_Elbow_width1").val());
            var Rec_Elbow_depth1 = parseFloat($("#Rec_Elbow_depth1").val());
            var Rec_Elbow_width2 = parseFloat($("#Rec_Elbow_width2").val());
            var Rec_Elbow_depth2 = parseFloat($("#Rec_Elbow_depth2").val());
            var Rec_Elbow_length = parseFloat($("#Rec_Elbow_length").val());
            var result =  (2*(Rec_Elbow_width1+Rec_Elbow_depth1)*Rec_Elbow_length-((Rec_Elbow_width1+Rec_Elbow_depth1)-(Rec_Elbow_width2+Rec_Elbow_depth2))*Rec_Elbow_length)/1000000;
            // area change
        ...
        $('select#Rec_Elbow-Thickness').on('change', function() {
            ...
            var result1 = (2*(Rec_Elbow_width1+Rec_Elbow_depth1+this.value*4)*Rec_Elbow_length-
            ((Rec_Elbow_width1+Rec_Elbow_depth1+this.value*4)-(Rec_Elbow_width2+Rec_Elbow_depth2+this.value*4))*Rec_Elbow_length)/1000000;
            if ( this.value == 25 ) {
...
<script>
    ...
    for (let element of [0,1,2,3,4,5,7,10,12,13] ) {
