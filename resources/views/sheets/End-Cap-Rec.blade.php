        ...
        <form action="{{route('EndCapRec')}}" method="post">
            ...
            <div class="row">
                ...
                <label class="block text-sm col-md-1">
                    ...
                    <input type="number" min="0" value="0" id="end-cap-rec-Area_1_inch" readonly name="Area_1_inch"
                        class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input">
                </label>
                <label class="block text-sm col-md-1">
                    <span class="text-gray-700 dark:text-gray-400">Area 2 inch</span>
                    <input type="number" min="0" value="0" id="end-cap-rec-Area_2_inch" readonly name="Area_2_inch"
                        class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input">
                </label>
                <label class="block text-sm col-md-1">
                    <span class="text-gray-700 dark:text-gray-400">Cladding</span>
                    <select id="end-cap-rec-Cladding" name="cladding_option"
                        class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                ...
                <label class="block text-sm col-md-1">
                    <span class="text-gray-700 dark:text-gray-400">Area (m^2)</span>
                    <input type="number" min="0" value="0" id="end-cap-rec-Cladding_Area" readonly name="Cladding_Area"
                        class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input">
                </label>
                <label class="block text-sm col-md-1">
                    <span class="text-gray-700 dark:text-gray-400">Duct Gage</span>
                    <select id="end-cap-rec-duct_gage" name="duct_gage"
                ...
                <label class="block text-sm col-md-1">
                    <span class="text-gray-700 dark:text-gray-400">Thickness</span>
                    <input type="number" readonly id="end-cap-rec-thickness" name="thickness" value="0.853"
                ...
                <label class="block text-sm col-md-1">
                    ...
                    <input type="number" readonly id="end-cap-rec-duct_weight" name="duct_weight" value="0"
            ...
            <div class="table-responsive">
                <table class="table table-bordered mb-0 text-left">
                    ...
                    <tbody class=" dark:bg-gray-800">
                        @forelse ($RecDucts->EndCapRecs as $EndCapRec)
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="cell">{{$EndCapRec->id ?? ''}}</td>

                            <td class="cell">{{$EndCapRec->width ?? ''}}</td>
                            <td class="cell">{{$EndCapRec->depth ?? ''}}</td>
                            <td class="cell">{{$EndCapRec->area ?? ''}}</td>
                            <td class="cell">{{$EndCapRec->thermal_thickness ?? ''}}</td>
                            <td class="cell">{{$EndCapRec->area_1_inch ?? ''}}</td>
                            <td class="cell">{{$EndCapRec->area_2_inch ?? ''}}</td>
                            <td class="cell">{{$EndCapRec->cladding_option == 1 ? 'yes': 'no'}}</td>
                            <td class="cell">{{$EndCapRec->cladding_area ?? ''}}</td>
                            <td class="cell">{{$EndCapRec->duct_gage ?? ''}}</td>
                            <td class="cell">{{$EndCapRec->thickness ?? ''}}</td>
                            <td class="cell">{{$EndCapRec->duct_weight ?? ''}}</td>
...
@push('scripts')
    <script>//-- 
        ...
        $("input[type='number']").change(function(){
            var end_cap_rec_width = parseInt($("#end-cap-rec-width").val());
            var end_cap_rec_depth = parseInt($("#end-cap-rec-depth").val());
            var result = ( end_cap_rec_width * end_cap_rec_depth / 1000000 );            
            // area change 
            $( "#end-cap-rec-area" ).val(result.toFixed(2));
            // duct_weight change
            $( "#end-cap-rec-duct_weight" ).val($("#end-cap-rec-area").val() * duct_weight[$("#end-cap-rec-duct_gage").val()]);
            //-------------------
        });
        $("#end-cap-rec-duct_gage").on('change', function() {
            // thickness duct_weight change in  duct_gage change
            $('#end-cap-rec-thickness').val(thickness[this.value]);
            $( "#end-cap-rec-duct_weight" ).val($("#end-cap-rec-area").val() * duct_weight[$("#end-cap-rec-duct_gage").val()]);
        });
        // -------------------------------
        // --------------------------------
        $('select#end-cap-rec-Thickness').on('change', function() {
            var end_cap_rec_width = parseInt($("#end-cap-rec-width").val());
            var end_cap_rec_depth = parseInt($("#end-cap-rec-depth").val());
            var result1 = ( end_cap_rec_width * end_cap_rec_depth / 1000000 );
            if ( this.value == 25 ) {
                $( "#end-cap-rec-Area_1_inch" ).val(result1.toFixed(2));
                $( "#end-cap-rec-Area_2_inch" ).val(0);
            }else if(this.value == 50){
                $( "#end-cap-rec-Area_2_inch" ).val(result1.toFixed(2));
                $( "#end-cap-rec-Area_1_inch" ).val(0);
            }else{
                $( "#end-cap-rec-Area_1_inch" ).val(0);
                $( "#end-cap-rec-Area_2_inch" ).val(0);
            }
        ...
        // -----------------------------------
        $('select#end-cap-rec-Cladding').on('change', function() {
            var resultCladding = 0 ;
            if ( this.value == 1 ) {
                if ($("select#end-cap-rec-Thickness").val() == '') {
                    resultCladding =  $("#end-cap-rec-area").val() * 1.006 ;
                    $( "#end-cap-rec-Cladding_Area" ).val(resultCladding);
                } else {
                    resultCladding = ($("#end-cap-rec-Area_1_inch" ).val() * 1.006) + ($("#end-cap-rec-Area_2_inch" ).val() * 1.006) ;
                    $( "#end-cap-rec-Cladding_Area" ).val(resultCladding.toFixed(4));
                }
            }else{
                $( "#end-cap-rec-Cladding_Area" ).val(resultCladding);
            }
